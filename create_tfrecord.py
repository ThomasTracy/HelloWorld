...
tf.app.flags.DEFINE_string('data_dir', '/home/tracy/data/TrafficSign/', 'Location of root directory for the '
                           ...
                           '(images).')
tf.app.flags.DEFINE_string('output_path', '/home/tracy/data/TrafficSign/traffic_sign', 'Path to which TFRecord files'
...
tf.app.flags.DEFINE_string('label_map_path', '/home/tracy/models/research/object_detection/data/traffic_sign_label_map.pbtxt',
...
def convert_kitti_to_tfrecords(data_dir, output_path,
  ...
  for img_name in images:
    ...
    anno_path = os.path.join(annotation_dir, str(img_num).zfill(5) + '.txt')
    if os.path.exists(anno_path):
        img_anno = read_annotation_file(anno_path)
    else:
    ...
    example = prepare_example(image_path, img_anno, label_map_dict)
...
def prepare_example(image_path, annotations, label_map_dict):
  ...
  encoded_png_io = io.BytesIO(encoded_jpg)
...
def read_annotation_file(filename):
  ...
  anno = {}
  anno['type'] = np.array([x[4].lower() for x in content])
  ...
  anno['2d_bbox_left'] = np.array([float(x[0]) for x in content])
  anno['2d_bbox_top'] = np.array([float(x[1]) for x in content])
  anno['2d_bbox_right'] = np.array([float(x[2]) for x in content])
...
def convert_txt():
    txt_path = '/home/tracy/data/TrafficSign/gt.txt'
    save_path = '/home/tracy/data/TrafficSign/Annotations'
    traffic_map = label_map_util.get_label_map_dict(FLAGS.label_map_path)
    with open(txt_path, 'r') as f:
        contents = f.readlines()
        for content in contents:
            content = content.strip().split(';')
            name = content[0].split('.')[0] + '.txt'
            save_name = os.path.join(save_path, name)
            with open(save_name, 'a') as f_w:
                f_w.write(' '.join(content[1:-1]))
                f_w.write(' ' + list(traffic_map.keys())[list(traffic_map.values()).index(int(content[-1]))+1])
                f_w.write('\n')
